...
import com.dlwx.onedrop.adapter.CommissionAdapter;
...
public class MyCommissionActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener {
    ...
    @BindView(R.id.rb_everymonth)
    RadioButton rbEverymonth;
    @BindView(R.id.rb_histhory)
    RadioButton rbHisthory;
    ...
    public void onCheckedChanged(RadioGroup radioGroup, int i) {
             switch (i){
                        case R.id.rb_everyday://每天
                            getData("1");
                            break;
                        case R.id.rb_everymonth://每月
                            getData("2");
                            break;
                        case R.id.rb_histhory://历史数据
                            getData("3");
                            break;
    ...
    private void getData(String time) {
        ...
        map.put("token",Token);
        map.put("time",time);
        mPreenter.fetch(map,true, HtttpUrlUtils.MyCommiss,HtttpUrlUtils.MyCommiss+Token);
    }
    ...
    public void showData(String s) {
        ...
        if (myCommissBean.getCode() == 200) {
            ...
            if ( myCommissBean.getBody() != null) {

                adapter = new CommissionAdapter(ctx,body);
                lvList.setAdapter(adapter);
                iv_entry.setVisibility(View.GONE);
